...
BlockLexer.prototype.captureTable = function(noLeadingPipe) {
  ...
  if (cap) {
    ...
    headerToken.type = 'table_header';
    tableToken.push(headerToken);
    ...
    for (var j = 0; j < cells.length; j++) {
      ...
      rowToken.type = 'table_row';
      tableToken.push(rowToken);
    }
...
BlockLexer.prototype.captureHR = function() {
  cap = this.capture(this.rules.hr);
  if (cap) {
    return {
      type: 'hr',
    };
...
BlockLexer.prototype.captureList = function() {
  ...
  if (cap) {
    ...
    for (var i = 0, l = cap.length; i < l; i++) {
      ...
      // Recurse.
      var lexer = new BlockLexer(this, { topLevel: false });
      var itemToken = lexer.tokenize(item);
      itemToken.type = loose
        ? 'loose_item'
        : 'list_item';
      listToken.push(itemToken);
    }
    listToken.type = 'list';
    listToken.ordered = bull.length > 1;
    return listToken;
  }
}
...
BlockLexer.prototype.captureParagraph = function() {
  ...
  if (cap) {
    return {
      type: 'paragraph',
      markdown: cap[1].charAt(cap[1].length - 1) === '\n'
        ? cap[1].slice(0, -1)
        : cap[1],
    };
  }
...
BlockLexer.prototype.capture = function(regExp) {
  var res = regExp.exec(this.remaining);
  if (res) {
    this.remaining = this.remaining.substr(res[0].length);
    return res;
  }
}

BlockLexer.prototype.backpedal = function(text) {
  this.remaining = backpedal + this.remaining;
}
